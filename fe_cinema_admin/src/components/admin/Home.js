      ...
      <Container fluid>
        ...
        <Row className="mb-4">
          ...
          <Col md={3}>
            <Card
              border="success"
              className="p-3 text-start shadow-sm border-2"
            >
        ...
        <Row className="mb-2">
                  ...
                  options={{
                    ...
                    plugins: {
                      ...
                      tooltip: {
                        callbacks: {
                          label: (context) =>
                            `${context.parsed.y.toLocaleString("vi-VN")}đ`,
                        },
                      },
                    },
                    scales: {
          ...
          <Col md={5}>
            <Card className="p-3 shadow-sm history-card">
              <span className="fs-5 mb-2 fw-bold text-start history-title">
                Lịch sử đặt vé
              </span>
                      ...
                      historybooking.map((hb, index) => (
                        <tr key={hb.id || hb.created_at}>
                          ...
                          <td>
                            <Badge
                              pill
                              bg={hb.status === "paid" ? "success" : "danger"}
                            >
                              {hb.status === "paid"
                                ? "Đã thanh toán"
                                : "Huỷ vé"}
                    ...
                    {topmovie && topmovie.length > 0 ? (
                      topmovie.map((top) => (
                        <tr key={top.id}>
                          ...
                          <td>
                            <span className="seat_occupancy_rate">
                              {parseFloat(top.seat_occupancy_rate).toFixed(1)}%
                            </span>
                          </td>
                          <td className="total-price text-success">
                            {formatCurrency(top.total_revenue)}
                          </td>
                        </tr>
                      ))
                    ) : (
